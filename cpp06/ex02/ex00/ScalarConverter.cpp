...
/*   Updated: 2026/01/03 12:39:37 by abhimi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScalarConverter::toChar(const std::string& str)
{
    ...
    if (!ScalarConverter::isvalidlitral(n,end) || n < 0  || n > 127)
        std::cout<<"impossible" <<"\n";
...
bool ScalarConverter::isvalidlitral(double n, char* end)
{
    if (*end == 'f' && *(end + 1) == '\0')
        ;
    else if (*end != '\0' || *(end-1) == '.')
        return false;
    if (n > INT_MAX  || n < INT_MIN || std::isnan(n))
       return false;
    return true;
}
...
void ScalarConverter::toInt(const std::string& str)
{
    ...
    char* end = NULL;
    double n = std::strtod(str.c_str(), &end);
    if (!ScalarConverter::isvalidlitral(n,end) || str.empty())
    {
        std::cout << "impossible" <<"\n";
        return;
    }
...
void ScalarConverter::toFloat(const std::string& str)
{
    std::cout << "Float: ";
        ...
        if(std::isinf(static_cast<float>(n)))
        {
            if (isNumber(str))
                std::cout << "impossible" <<"\n";
            else 
